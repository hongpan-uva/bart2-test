...
def main(args):
    ...
    '''
    if hasattr(args, "samplefile"):
        samplefile = args.samplefile
        sample_names, sample_weights = read_sample_list(samplefile)
    elif hasattr(args, "sample_df"):
        sample_df = args.sample_df
        sample_names = sample_df.loc[:, "ID"].tolist()
        sample_weights = sample_df.loc[:, "coef"].tolist()

    output_name = args.name
    H3K27ac_hdf5 = args.k27achdf5

    # incase 'start/end/chr' in sample names
    sample_names = [i for i in sample_names if re.match('[0-9]', i)]
    ...
    for i, elem in enumerate(T):
        out_res.append((chrom[i].decode('utf-8'),
                       start[i], end[i], str(i+1), elem))
    sorted_out_res = sorted(out_res, key=lambda x: float(x[4]), reverse=True)

    sorted_out_res_df = pd.DataFrame(np.array(sorted_out_res), columns=[
                                     'chromosom', 'start', 'end', 'UDHSID', 'Score'])
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description="""Predict enhancer elements from gene list and H3K27ac in union DNase-seq peaks.""")
    parser.add_argument('-s', dest='samplefile', type=str, required=True,
                        help='File that lists informative H3K27ac samples. One sample ID per line.')
    parser.add_argument('--k27ac', dest='k27achdf5', type=str, required=True,
                        help='Path for the hdf5 format H3K27ac reads counts in UDHS regions.')
    parser.add_argument('-n', dest='name', type=str, required=True,
                        help='Name of study, for output file naming.')
