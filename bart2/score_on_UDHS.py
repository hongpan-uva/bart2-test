...
def score_on_DHS(args):
    ...
    udhs_starts, udhs_ends, udhs_ids = read_regions_from_bed_not_sep_strand(
        args.dhsfile, args.species)
    ...
    if hasattr(args, "infile"):
        ...
        while line:
            ...
            if chrom in udhs_starts:
                ...
                if start < end:
                    overlapped_ids = get_overlapped_udhs(
                        start, end, udhs_starts[chrom], udhs_ends[chrom], udhs_ids[chrom])
                else:
                    sys.stdout.write(
                        "Warning: start positions should be smaller than end positions!\n")
    ...
    elif hasattr(args, "in_df"):
        bed_df = args.in_df
        for row in range(0, len(bed_df.index)):
            chrom = bed_df.iloc[row, 0]
            start = int(bed_df.iloc[row, 1])
            end = int(bed_df.iloc[row, 2])
            # score from specific column
            score = float(bed_df.iloc[row, (args.scorecol-1)])
            if chrom in udhs_starts:
                # get the score on this DHS, 0 if not overlapping with inpit bed file
                if start < end:
                    overlapped_ids = get_overlapped_udhs(
