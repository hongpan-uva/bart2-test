...
def gene_sym(symfile):
    ...
    fp = open(symfile)
    # {"gene_symbol": ["refseq_ID", "chr"]}  the first refseq ID in the TSS file
    symdict = {}
    ...
    for k, name in enumerate(samplenames):
        ...
        for i, geneid in enumerate(refseqID):
            ...
            if geneid in sym:
                ...
                rp = RP[i]
                try:
                    # float num was ignored here, e.g., 'chr', 'refseqID', 'start', 'symbol'
                    x[index[symid], k] = rp
                except:
                    pass
...
def read_genelistOnly(sym, index, exptype, fname=None, genes=None):
    ...
    test_chroms = ['chr2', 'chr4', 'chr6', 'chr8', 'chr10',
                   'chr12', 'chr14', 'chr16', 'chr18', 'chr20', 'chr22']
    train_index = []
    test_index = []
...
def dataset_annotation(annotationf):
    ...
    for line in inf:
        if line.startswith('datasetID'):
            ...
            # CellLineName, Tissue/Organ, DetailedTissue
...
def lasso_test_best_alpha(x, y, prename):
    ...
    plt.figure()
    for alpha in alphas:
        LR_l1 = linear_model.LogisticRegression(
            penalty='l1', tol=0.01, fit_intercept=True, C=alpha)
        print(alpha)
        cvs_scores = model_selection.cross_val_score(
            LR_l1, x, y, scoring='roc_auc', cv=5)
        ...
        plt.errorbar(alpha, y_mean, y_err, color='r',
                     ecolor='grey', fmt='o', capsize=4)
    plt.ylim([0, 1])
    plt.xscale('log')
    plt.savefig(prename+'_alpha_auc.png', bbox_inches='tight',
...
def adaptive_lasso(x, y, samplefiles, name, maxsamples, ann, genenames):
    ...
    def g(w): return np.sqrt(np.abs(w))
    def gprime(w): return 1.0/(2.*np.sqrt(np.abs(w))+np.finfo(float).eps)
    ...
    for k in range(n_lasso_iterations):
        ...
        estimator = linear_model.LogisticRegression(
            penalty='l1', tol=0.01, fit_intercept=True, C=alpha, random_state=2019, solver="liblinear")
        estimator.fit(X_w, y)
    ...
    cvs_scores = model_selection.cross_val_score(
        estimator, xt, yt,  scoring='roc_auc', cv=5)
...
#    print(k,'selected_features',selected_features)
    ...
    out_df = pd.DataFrame(columns=["ID", "coef", "anno_1", "anno_2", "anno_3"])
    for coef in sorted([i for i in coef_[0] if i != 0], key=abs, reverse=True):
        ...
        if dataID in list(ann.keys()):
            ...
            annInfo = ['NA', 'NA', 'NA']
        out_df.loc[len(out_df.index)] = [dataID, coef,
                                         annInfo[0], annInfo[1], annInfo[2]]

    summary = [auc, best_cvs, selected_features]

    return auc, selected_features, out_df, summary
...
def main(args):
    ...
    try:
        auc, selected_features, model_df, model_summary = adaptive_lasso(
            x, y, z.rpfiles, name, maxsamples, ann, genenames)
    except:
...
Please check whether you selected the correct species or uploaded the correct gene list!\n""")
    ...
    return model_df, model_summary


if __name__ == "__main__":
    try:
        parser = argparse.ArgumentParser(
            description="""Regression of regulatory potential to gene expression changes.""")
        # related to input file type
        parser.add_argument('-e', '--expr', dest='expr', required=True,
                            type=str, help='The related differential expression file')
        ...
        parser.add_argument('-a', dest='annotation', required=True,
                            type=str, help='The annotation file for each dataset')

        parser.add_argument('-m', dest='sym', type=str, required=True,
                            help='refseqTSS is six columns: <chromosome name> <TSS> <TSS+1> <refseq:genesymbok> <score> <strand>')
        ...
        parser.add_argument('-g', '--genome', dest="genome", type=str, default='hg38',
                            choices=['mm9', 'hg19', 'hg38', 'mm10'], required=False, help='genome')
        ...
        parser.add_argument('-n', '--name', dest='name', required=True,
                            type=str, help='The prefix of the output names')
