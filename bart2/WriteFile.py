...
def write_udhs_prediction(args, df):
    ...
    for i in range(0, len(df.index)):
        ...
        outf.write('{}\t{}\t{}\t{}\t{:.2f}\n'.format(
            out_line[0], out_line[1], out_line[2], out_line[3], out_line[4]))
...
def write_bart_result(args, df):
    ...
    outf.write('TR\t{}\t{}\t{}\t{}\t{}\t{}\n'.format(
        'statistic', 'pvalue', 'zscore', 'max_auc', 're_rank', 'irwin_hall_pvalue'))
    for i in df.index:
        outf.write('{}\t{:.3f}\t{:.3e}\t{:.3f}\t{:.3f}\t{:.3f}\t{:.3e}\n'.format(
            i, df.loc[i]['score'], df.loc[i]['pvalue'], df.loc[i]['zscore'], df.loc[i]['max_auc'], df.loc[i]['rank_avg_z_p_a'], df.loc[i]['rank_avg_z_p_a_irwinhall_pvalue']))
    sys.stdout.write('--Standardization finished!\n--Ranked TRs saved in file: {}\n'.format(
        args.ofilename + '_bart_results.txt'))
